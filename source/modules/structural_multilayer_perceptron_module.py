...
from source.metrics import prepare_masks_per_step

UNCERTAINTY_ESTIMATE_NAMES = ['data', 'knowledge', 'total']


class StructuralMultilayerPerceptronLightningModule(BaseLightningModule):
    ...
    def setup_model(self):
        model_class = get_model_class('structural_multilayer_perceptron')
    ...
    def forward(self, batch):
        ...
        return self.model.forward(graph, node_features, structural_features)

    def get_data_uncertainty(self, batch):
        graph, node_features, structural_features, labels, masks, step_name = batch
        return self.model.get_data_uncertainty(graph, node_features, structural_features)

    def get_knowledge_uncertainty(self, batch):
        graph, node_features, structural_features, labels, masks, step_name = batch
        return self.model.get_knowledge_uncertainty(graph, node_features, structural_features)

    def get_total_uncertainty(self, batch):
        graph, node_features, structural_features, labels, masks, step_name = batch
        return self.model.get_total_uncertainty(graph, node_features, structural_features)
